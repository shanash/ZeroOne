...
public class BattleNpcOnetimeSkillData_PhysicsDamage : BattleNpcOnetimeSkillData
{
    BattleNpcOnetimeSkillData_PhysicsDamage() { }
        ...
        for (int i = 0; i < cnt; i++)
        {
            ...
            switch (GetStatMultipleType())
            {
                case STAT_MULTIPLE_TYPE.ATTACK_VALUE:   //  절대값
                    send_data.Physics_Attack_Point = GetValue();
                    break;
                case STAT_MULTIPLE_TYPE.ATTACK_RATE:
                    send_data.Physics_Attack_Point = send_data.Caster.Physics_Attack * GetMultiple();
                    break;
                case STAT_MULTIPLE_TYPE.DEFENSE_VALUE:
                    send_data.Physics_Attack_Point = send_data.Caster.Physics_Defense * GetMultiple();
                    break;
                case STAT_MULTIPLE_TYPE.MAX_LIFE:
                    send_data.Physics_Attack_Point = send_data.Caster.Max_Life * GetMultiple();
                    break;
                case STAT_MULTIPLE_TYPE.LIFE:
                    send_data.Physics_Attack_Point = send_data.Caster.Life * GetMultiple();
                    break;
