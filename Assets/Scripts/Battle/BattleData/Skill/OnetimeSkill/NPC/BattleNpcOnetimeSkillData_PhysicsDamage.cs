        ...
        for (int i = 0; i < cnt; i++)
        {
            ...
            switch (GetStatMultipleType())
            {
                case STAT_MULTIPLE_TYPE.ATTACK_RATE:
                    send_data.Physics_Attack_Point = send_data.Caster.Physics_Attack * GetMultiple() + GetValue();
                    break;
                case STAT_MULTIPLE_TYPE.DEFENSE_RATE:
                    send_data.Physics_Attack_Point = send_data.Caster.Physics_Defense * GetMultiple() + GetValue();
                    break;
                case STAT_MULTIPLE_TYPE.MAX_LIFE_RATE:
                    send_data.Physics_Attack_Point = send_data.Caster.Max_Life * GetMultiple() + GetValue();
                    break;
                case STAT_MULTIPLE_TYPE.LIFE_RATE:
                    send_data.Physics_Attack_Point = send_data.Caster.Life * GetMultiple() + GetValue();
                    break;
