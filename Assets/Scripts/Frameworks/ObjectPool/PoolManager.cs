...
namespace FluffyDuck.Util
{
    ...
    public class PoolManager : IDisposable
    {
        ...
        public GameObject Pop(string path, Transform parent, Vector3 pos)
        {
            string name = System.IO.Path.GetFileNameWithoutExtension(path);
            GameObject item = null;
            if (Recycled_Objects.Count > 0)
        ...
        /// <summary>
        /// GameObject 비동기로 가져오기
        /// </summary>
        /// <param name="path">경로</param>
        /// <param name="parent">하이라키의 부모</param>
        public async Task<GameObject> PopAsync(String path, Transform parent)
        {
            string name = System.IO.Path.GetFileNameWithoutExtension(path);
            GameObject item = null;
            if (Recycled_Objects.Count > 0)
        ...
        public async Task<int> PreLoadAsset(string path)
        {
            Handle = Addressables.LoadAssetAsync<GameObject>(path);
            Object_Prefab = await Handle.Task;
